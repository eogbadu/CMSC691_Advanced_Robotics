...
calculate_confidence_interval <- function(correct, total, confidence_level = 0.95) {
  ci <- prop.test(correct, total, conf.level = confidence_level)$conf.int
  
  # Return a named list with lower and upper bounds
...
by_real_output_analyze <- function(mturk_results) {
  ...
  grouped_results <- mturk_results %>%
    ...
    grouped_results <- grouped_results %>%
      rowwise() %>%
      mutate(
      CI = calculate_confidence_interval(matches_with_image, total_with_image),
      CI_With_lower = CI$lower,
      CI_With_upper = CI$upper,
      CI = calculate_confidence_interval(matches_without_image, total_without_image),
      CI_Without_lower = CI$lower,
      CI_Without_upper = CI$upper
    ) %>%
    mutate(
      `Accuracy %_With` = sprintf("%.2f%% (%.2f%% - %.2f%%)", percentage_with_image, CI_With_lower * 100, CI_With_upper * 100),
      `Accuracy %_Without` = sprintf("%.2f%% (%.2f%% - %.2f%%)", percentage_without_image, CI_Without_lower * 100, CI_Without_upper * 100)
    ) %>%
    select(-CI_With_lower, -CI_With_upper, -CI_Without_lower, -CI_Without_upper)
  
    #   `Accuracy %_With`,
    #   `No. Correct_With` = matches_with_image,
    #   `No. Total_With` = total_with_image,
    #   `P Value` = p_value
    # )
  ...
  #          `Accuracy %_With`, `No. Correct_With`, `No. Total_With`, `P Value`)
  # 
  # 
...
#       percentage_without_image = (matches_without_image / total_without_image) * 100,
#       p_value = ifelse(total_with_image >= 5 & total_without_image >= 5,
#                        round(chisq.test(matrix(c(matches_with_image, total_with_image - matches_with_image,
#                                                  matches_without_image, total_without_image - matches_without_image), ncol = 2))$p.value, 3),
#                        NA_real_)
#     ) %>%
#     transmute(
...
print(head(by_real_output_table))
